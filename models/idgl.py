...
class IDGL(nn.Module):
    def __init__(self, config, nfeat, nclass):
        super(IDGL, self).__init__()
        self.nfeat = nfeat
        self.nclass = nclass
        self.hidden_size = config.hidden_size
        self.dropout = config.dropout
        self.scalable_run = config.scalable_run if 'scalable_run' in config else False
        self.feat_adj_dropout = config.feat_adj_dropout

        gcn_module = AnchorGCN if self.scalable_run else GCN
        self.encoder = gcn_module(nfeat=nfeat, nhid=self.hidden_size, nclass=nclass, n_layers=config.n_layers, dropout=self.dropout, with_bias=False, batch_norm=config.batch_norm)
